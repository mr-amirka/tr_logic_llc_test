...
Загрузка изображений осуществляется методом POST для [http://0.0.0.0:8000/images](http://0.0.0.0:8000/images)  
...
4. Загрузка нескольких изображений в JSON формате с BASE64 закодированным содержимом (Content-Type: application/json).  
Например:  
```js
[
  {
...
5. Загрузка нескольких изображений в текстовом формате с BASE64 закодированным содержимом или со сторонних ресурсов по заданному URL, разделенных переводом строки (Content-Type: text/plain).  
Например:  
```
...
Получение изображения осуществляется методом GET для [http://0.0.0.0:8000/images](http://0.0.0.0:8000/images)  
...
Получение квадратного превью изображения осуществляется методом GET для [http://0.0.0.0:8000/thumbnails](http://0.0.0.0:8000/thumbnails)  
...
Front-end приложение для тестирования серверной части доступно по адресу [http://0.0.0.0:8000/](http://0.0.0.0:8000/)  
К Front-end приложению относятся также ресурсы:
- [http://0.0.0.0:8000/assets/app.css](http://0.0.0.0:8000/assets/app.css);
- [http://0.0.0.0:8000/assets/app.js](http://0.0.0.0:8000/assets/app.js);
- [http://0.0.0.0:8000/assets/environment.js](http://0.0.0.0:8000/assets/environment.js).
